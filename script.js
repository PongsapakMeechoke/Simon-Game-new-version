//  get DOM element needed for game
...
const showPath = async (colors) =>{
    ...
    for(let color of colors){
        ...
        // pause execution for 500 millseconds
        await delay(200);
...
const endGame = () =>{
    resultEl.innerHTML = `<span> Yore Score : </span> ${score}`;
...
// function to rest game after ending
...
const handleColorClick = async (e) =>{
    ...
    if(e.target.classList.contains(randomColors[clickCount])){
        ...
        // if the clicked is incorrect, end game
    }else{
        endGame();
    }
};
